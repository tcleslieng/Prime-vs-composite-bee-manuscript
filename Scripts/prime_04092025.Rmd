---
title: "Exploring discrimination of symmetrical/asymmetrical quantities in honeybees using perceptual grouping"
...
EXT_7v9 <- subset(EXT_PRIMING, TEST == "7v9") # Experiment 2 data
EXT_9v11 <- subset(EXT_PRIMING, TEST == "11v9") 
EXT_13v15 <- subset(EXT_PRIMING, TEST == "13v15") 

EXT2_LEARNING <- subset(EXT_TRAINING, TEST == "Learning") # Experiment 6 data
...
#Subset Experiment 2 training data
...
# Treat variables as factor

EXT_TRAINING$TEST <- as.factor(EXT_TRAINING$TEST)
EXT_TRAINING$BEEID <- as.factor(EXT_TRAINING$BEEID)
EXT_PRIMING$BEEID <- as.factor(EXT_PRIMING$BEEID)
EXT_TRAINING_trainingphase$BEEID <- as.factor(EXT_TRAINING_trainingphase$BEEID)
TRAINING_trainingphase$BEEID <- as.factor(TRAINING_trainingphase$BEEID)
levels(EXT_TRAINING$TEST)
...
lm_trainingphase <-glmer(CHOICE~ CNUM + (1|BEEID), family = binomial, data = TRAINING_trainingphase) 
summary(lm_trainingphase)
...
confint(lm_trainingphase)
trainingphaseconfintlow <-(exp(-0.002835008)/(1 + exp(-0.002835008)))
trainingphaseconfinthigh <-(exp(0.03981324)/(1 + exp(0.03981324)))
print(trainingphaseconfintlow)
print(trainingphaseconfinthigh)
...
dframeext2 <- data.frame(Test = rep(c("Learning", "7v9","7v8")), #create dataframe
                        percentage = c(0.62, 0.54, 0.58),
                        confintlow = c(ext4_confintlow, ext5_confintlow, ext6_confintlow),
                        confinthigh = c(ext4_confinthigh, ext5_confinthigh, ext6_confinthigh))
dframeext2$Test <- factor(dframeext2$Test, levels=c('Learning', '7v9', '7v8'))

ext2points <- EXT_TRAINING %>%   # calculate mean of each block for each bee
  group_by(BEEID, TEST) %>% 
  summarize(prop = mean(CHOICE))
ext2points$TEST <- factor(ext2points$TEST, levels=c('Learning', '7v9', '7v8')) #
levels(ext2points$TEST) # check to view the change
levels(dframeext2$Test) 
...
trngraph <- ggplot()+
  ...
  geom_bar(data = dframetrn, aes(x = Test, y = percentage, fill = Test), stat = "identity", position = "dodge") + 
   geom_point(data = trainingpoints, aes(x = CONDITION, y = prop, col = BEEID), position = position_jitter(width=0.15,height=0.01), size =2) +
...
ext2graph <- ggplot() +
  ...
  geom_point(data = ext2points, aes(x = TEST, y = prop, col = BEEID), position = position_jitter(width = 0.15, height = 0.01), size = 2) +
  ...
  geom_hline(yintercept = 0.5, linetype = "dashed", colour = "black") +
  # Add asterisk explicitly for Learning condition at y = 0.75
  geom_text(data = data.frame(Test = "Learning", percentage = 0.9), 
            aes(x = Test, y = percentage, label = "*"), 
            size = 10, color = "black") +
  scale_fill_manual(values = wes_palette("GrandBudapest2", n = 3)) +
  scale_x_discrete(labels = c("Learning" = "Learning", "7v9" = "7 vs 9", "7v8" = "7 vs 8")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1, suffix = NULL), limits = c(0, 1)) +
  theme(panel.background = element_blank(),
        axis.line = element_line(colour = "black"),
        aspect.ratio = 1/1,
...
pred_ext <- predict(lm_trainingphase, newdata = new_data_ext, type = "link", se.fit = TRUE, re.form = NA)

new_data_ext$fit <- plogis(pred_ext$fit)
new_data_ext$lwr <- plogis(pred_ext$fit - 1.96 * pred_ext$se.fit)
new_data_ext$upr <- plogis(pred_ext$fit + 1.96 * pred_ext$se.fit)
...
ggsave("p_ext5.png", width = 5, height = 5, dpi = 300, path = "../Output")
ggsave("p_ext5.png", width = 5, height = 5, dpi = 300, path = "../Output")
...
fig5b <- trngraph
fig5b
ggsave("fig5b.pdf", device = pdf, height= 4, width=5, dpi = 300, path = "../Output")
ggsave("fig5b.png", device = png, height= 4, width=5, dpi = 300, path = "../Output")

fig5c <- ext2graph
fig5c
ggsave("fig5c.pdf", device = pdf, height= 4, width=5, dpi = 300, path = "../Output")
ggsave("fig5c.png", device = png, height= 4, width=5, dpi = 300, path = "../Output")
