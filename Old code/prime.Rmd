...
## Training + priming experiments (60 trials)
...
# Create dataframe for 60 trials priming experiment
summary(EXT_7v9)
summary(EXT_9v11)
summary(EXT_13v15)
...
# Create dataframe for 60 trials training experiment
...
## Learning phase graph for 60 trials of training

```{r message=FALSE, error=FALSE}
# Step 1: Create new data frame for prediction
new_data_ext <- data.frame(
  CNUM = seq(min(EXT_TRAINING_trainingphase$CNUM), max(EXT_TRAINING_trainingphase$CNUM), length.out = 100)
)

# Add dummy BEEID to satisfy model's random effect structure
new_data_ext$BEEID <- NA

# Step 2: Predict using the model, excluding random effects (re.form = NA)
pred_ext <- predict(lm_ext_training, newdata = new_data_ext, type = "link", se.fit = TRUE, re.form = NA)

# Step 3: Back-transform to response (probability) scale
...
# Save to files
ggsave("p_ext.png", width = 5, height = 5, dpi = 300, path = "../Output")
ggsave("p_ext.png", width = 5, height = 5, dpi = 300, path = "../Output")
...
figx3 <- ext1graph + ext2graph
figx3 + plot_annotation(tag_levels = "a") & 
  theme(plot.tag.position = c(-0.05, 1),plot.tag = element_text(size = 16, face = "bold"))
ggsave("fig3x.pdf", device = pdf, height= 4, width=8.2, dpi = 300, path = "../Output")
ggsave("fig3x.png", device = png, height= 4, width=8.2, dpi = 300, path = "../Output")
```
